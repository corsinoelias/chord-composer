...
function DurationDots({ duration, isActive, bpm, uid }: {
  duration: number; isActive: boolean; bpm: number; uid: number
}) {
  ...
  return (
    <span className={`flex items-center gap-[3px] mt-0.5 transition-opacity duration-100 ${isActive ? 'opacity-80' : 'opacity-25'}`}>
                        ...
                        {line.map((token, ti) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {hasChord && isPlaying && <DurationDots duration={token.duration} isActive={isActive} bpm={bpm} uid={token.globalIndex} />}
