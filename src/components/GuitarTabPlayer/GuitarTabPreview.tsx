...
import { type GuitarNote, type GuitarTrack, type GuitarSound } from '../../lib/guitarTab/types'
import { GuitarTabView } from './GuitarTabView'
import { startPlayback, stopPlayback } from '../../lib/guitarTab/guitarAudio'
...
export function GuitarTabPreview({ presetId, editorHref = '/guitar-tab/' }: Props) {
  const preset = GUITAR_PRESETS.find(p => p.id === presetId)

  const trackRef = useRef<GuitarTrack | null>(
    preset
      ? { id: preset.id, name: preset.name, bpm: preset.bpm, beatsPerBar: preset.beatsPerBar, totalBars: preset.totalBars, notes: preset.notes, capo: preset.capo, tuning: preset.tuning, sections: preset.sections }
      : null
  )
  ...
  const handleTogglePlay = useCallback(() => {
    ...
    startPlayback(
      trackRef.current,
      from,
      preset.defaultSound as GuitarSound,
  ...
  return (
    <div className="relative rounded-xl border border-border bg-background overflow-hidden" style={{ height: 244 }}>
      {/* Read-only tab notation — GuitarTabView auto-scrolls to keep the
          playhead in view, so we just constrain its height and disable
          pointer events/edits. */}
      <div className="absolute inset-0 bottom-14 overflow-hidden pointer-events-none select-none">
        <GuitarTabView
          ...
          isPlaying={isPlaying}
          selectedNoteId={null}
          sound={preset.defaultSound as GuitarSound}
          onAddNote={noop as unknown as (n: GuitarNote) => void}
          onUpdateNote={noop as unknown as (id: string, patch: Partial<GuitarNote>) => void}
          onDeleteNote={noop}
          onSelectNote={noop}
